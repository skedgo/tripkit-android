    ...
    fun saveCarParks(carParkEntity: List<Triple<CarParkLocationEntity, Map<OpeningDayEntity, List<OpeningTimeEntity>>, Map<PricingTableEntity, List<PricingEntryEntity>>>>): Completable {
        return Completable
            .fromAction {
                ...
                val openingTimes: List<OpeningTimeEntity> =
                    carParkEntity.map { it.second }.flatMap { it.values }.flatMap { it }
                val pricingTables = carParkEntity.map { it.third }.flatMap { it.keys }
                val pricingEntries =
                    carParkEntity.map { it.third }.flatMap { it.values }.flatMap { it }
                tripKitDatabase.carParkDao().saveAll(
                    carParkEntities,
                    openingDays,
                    openingTimes,
                    pricingTables,
