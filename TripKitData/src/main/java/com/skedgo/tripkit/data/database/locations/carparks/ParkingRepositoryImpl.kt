...
internal class ParkingRepositoryImpl(val api: LocationsApi,
                                     ...
                                     val stopsFetcher: StopsFetcher, val tripKitDatabase: TripKitDatabase) : ParkingRepository {

  override fun fetchCarParks(center: GeoPoint, cellIds: List<String>, zoomLevel: Int): Observable<List<OffStreetParking>> {
  ...
  private fun convert(it: CarParkLocationEntity): OffStreetParking {
    ...
    return OffStreetParking(it.identifier,
        it.name,
        GeoPoint(it.lat, it.lng),
        it.remoteIconName,
        it.localIconName,
        it.address,
        ParkingOperator(it.operator.name, it.operator.phone.toOptional(), it.operator.website.toOptional()),
        it.info.orEmpty(),
        openingHours,
        getPricingTablesByCarParkId(it.identifier)
            .map {
              PricingTable(it.currency,
                  it.currencySymbol,
                  this.getPricingEntriesByPricingTableId(it.id)
                      .map { PricingEntry(it.maxDurationInMinutes.toOptional(), it.label.orEmpty(), it.price) },
                  it.title,
