...
internal class PrintFullDateImpl constructor(
    ...
    private val printFullDateByJoda: (DateTime, Locale) -> String = { dateTime, locale ->
        DateTimeFormat.fullDate().withLocale(locale).print(dateTime)
    },
    private val printFullDateByJava: (Date, Locale) -> String = { date, locale ->
        DateFormat.getDateInstance(DateFormat.FULL, locale).format(date)
    }
) : PrintFullDate {
    override fun execute(dateTime: DateTime): Flowable<String> = Flowable
        .fromCallable {
