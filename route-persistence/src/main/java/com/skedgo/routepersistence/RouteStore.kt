...
import com.skedgo.routepersistence.RouteContract.COL_AVAILABILITY
import com.skedgo.routepersistence.RouteContract.COL_AVAILABILITY_INFO
...
import com.skedgo.routepersistence.RouteContract.COL_IS_HIDE_EXACT_TIMES
...
import com.skedgo.routepersistence.RouteContract.COL_LOG_URL
import com.skedgo.routepersistence.RouteContract.COL_MAIN_SEGMENT_HASH_CODE
import com.skedgo.routepersistence.RouteContract.COL_MONEY_COST
import com.skedgo.routepersistence.RouteContract.COL_MONEY_USD_COST
...
import com.skedgo.routepersistence.RouteContract.COL_QUERY_TIME
...
import com.skedgo.routepersistence.RouteContract.COL_SHARE_URL
import com.skedgo.routepersistence.RouteContract.COL_SOURCES
import com.skedgo.routepersistence.RouteContract.COL_SUBSCRIBE_URL
...
import com.skedgo.routepersistence.RouteContract.COL_UNSUBSCRIBE_URL
...
open class RouteStore(private val databaseHelper: SQLiteOpenHelper, private val gson: Gson) {
            ...
            .fromAction {
                ...
                try {
                    val cursor = database.rawQuery(
                        "SELECT MAX($COL_ID) FROM $TABLE_TRIPS WHERE $COL_GROUP_ID = ?",
                        arrayOf(tripGroupId)
                    )
                    cursor.moveToFirst()
    ...
    private fun asTrip(tripCursor: Cursor, groupCursor: Cursor): Trip {
        ...
        val queryIsLeaveAfter =
            tripCursor.getInt(tripCursor.getColumnIndex(COL_QUERY_IS_LEAVE_AFTER))
        val mainSegmentHashCode =
            tripCursor.getLong(tripCursor.getColumnIndex(COL_MAIN_SEGMENT_HASH_CODE))
        val shareUrl = tripCursor.getString(tripCursor.getColumnIndex(COL_SHARE_URL))
        val isHideExactTimes =
            tripCursor.getInt(tripCursor.getColumnIndex(COL_IS_HIDE_EXACT_TIMES)) == 1
        ...
        val availabilityInfo =
            tripCursor.getString(tripCursor.getColumnIndex(COL_AVAILABILITY_INFO))
    ...
    private fun asTripGroup(cursor: Cursor): TripGroup {
        ...
        val sources = gson.fromJson(
            cursor.getString(cursor.getColumnIndex(COL_SOURCES)),
    ...
    private fun saveTripGroup(
        ...
        if (group.sources != null) {
            values.put(
                COL_SOURCES,
                gson.toJson(group.sources!!.toTypedArray(), Array<Source>::class.java)
            )
        }
