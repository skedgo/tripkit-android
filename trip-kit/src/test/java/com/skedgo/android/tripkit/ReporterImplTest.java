...
public class ReporterImplTest {
  ...
  @Before public void before() {
    ...
    reporter = new ReporterImpl(api, errorHandler);
  }

  @Test public void shouldSubscribeErrorHandler() {
    final RuntimeException error = new RuntimeException();
    when(api.reportPlannedTripAsync(eq(EXPECTED_PLANNED_URL), eq(Collections.<String, Object>emptyMap())))
  ...
  @Test public void shouldNotRequestApi() {
    ...
    verify(api, times(0)).reportPlannedTripAsync(
        eq(EXPECTED_PLANNED_URL),
  ...
  @Test public void shouldUseEmptyMapAsUserInfo() {
    ...
    verify(api, times(1)).reportPlannedTripAsync(
        eq(EXPECTED_PLANNED_URL),
  ...
  @Test public void shouldAttachUserInfo() {
    ...
    when(api.reportPlannedTripAsync(eq(EXPECTED_PLANNED_URL), eq(userInfo)))
        .thenReturn(Observable.<JsonObject>empty());
