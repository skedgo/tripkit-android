...
import static com.skedgo.tripkit.common.util.MathUtil.EARTH_RADIUS;
import static com.skedgo.tripkit.common.util.MathUtil.clamp;
import static com.skedgo.tripkit.common.util.MathUtil.hav;
import static com.skedgo.tripkit.common.util.MathUtil.havDistance;
import static com.skedgo.tripkit.common.util.MathUtil.havFromSin;
import static com.skedgo.tripkit.common.util.MathUtil.inverseMercator;
import static com.skedgo.tripkit.common.util.MathUtil.mercator;
import static com.skedgo.tripkit.common.util.MathUtil.sinFromHav;
import static com.skedgo.tripkit.common.util.MathUtil.sinSumFromHav;
import static com.skedgo.tripkit.common.util.MathUtil.wrap;
import static com.skedgo.tripkit.common.util.SphericalUtil.computeDistanceBetween;
...
public class PolyUtil {
    ...
    public static boolean containsLocation(TripKitLatLng point, List<TripKitLatLng> polygon, boolean geodesic) {
     ...
     */
    public static boolean containsLocation(double latitude, double longitude, List<TripKitLatLng> polygon, boolean geodesic) {
        ...
        TripKitLatLng prev = polygon.get(size - 1);
        double lat1 = toRadians(prev.latitude);
        double lng1 = toRadians(prev.longitude);
        int nIntersect = 0;
        for (TripKitLatLng point2 : polygon) {
     ...
     */
    public static boolean isLocationOnEdge(TripKitLatLng point, List<TripKitLatLng> polygon, boolean geodesic,
                                           double tolerance) {
        return isLocationOnEdgeOrPath(point, polygon, true, geodesic, tolerance);
    }

    /**
     * Same as {@link #isLocationOnEdge(TripKitLatLng, List, boolean, double)}
     ...
     */
    public static boolean isLocationOnPath(TripKitLatLng point, List<TripKitLatLng> polyline,
                                           boolean geodesic, double tolerance) {
        return isLocationOnEdgeOrPath(point, polyline, false, geodesic, tolerance);
    }

    /**
     * Same as {@link #isLocationOnPath(TripKitLatLng, List, boolean, double)}
     * <p>
     * with a default tolerance of 0.1 meters.
     */
    public static boolean isLocationOnPath(TripKitLatLng point, List<TripKitLatLng> polyline,
    ...
    private static boolean isLocationOnEdgeOrPath(TripKitLatLng point, List<TripKitLatLng> poly, boolean closed,
     ...
     */
    public static int locationIndexOnPath(TripKitLatLng point, List<TripKitLatLng> poly,
                                          boolean geodesic, double tolerance) {
        return locationIndexOnEdgeOrPath(point, poly, false, geodesic, tolerance);
    }

    /**
     * Same as {@link #locationIndexOnPath(TripKitLatLng, List, boolean, double)}
     * <p>
     * with a default tolerance of 0.1 meters.
     */
    public static int locationIndexOnPath(TripKitLatLng point, List<TripKitLatLng> polyline,
                                          boolean geodesic) {
     ...
     * poly.size()-2 if between poly[poly.size() - 2] and poly[poly.size() - 1]
     */
    public static int locationIndexOnEdgeOrPath(TripKitLatLng point, List<TripKitLatLng> poly, boolean closed,
                                                boolean geodesic, double toleranceEarth) {
        int size = poly.size();
        if (size == 0) {
            return -1;
        ...
        TripKitLatLng prev = poly.get(closed ? size - 1 : 0);
        double lat1 = toRadians(prev.latitude);
        double lng1 = toRadians(prev.longitude);
        int idx = 0;
        if (geodesic) {
            for (TripKitLatLng point2 : poly) {
                double lat2 = toRadians(point2.latitude);
                double lng2 = toRadians(point2.longitude);
     ...
     */
    public static List<TripKitLatLng> simplify(List<TripKitLatLng> poly, double tolerance) {
        ...
        if (closedPolygon) {
            ...
            // TripKitLatLng.latitude and .longitude are immutable, so replace the last point
            poly.remove(poly.size() - 1);
            poly.add(new TripKitLatLng(lastPoint.latitude + OFFSET, lastPoint.longitude + OFFSET));
        }
        ...
        idx = 0;
        ArrayList<TripKitLatLng> simplifiedLine = new ArrayList<>();
        for (TripKitLatLng l : poly) {
            if (dists[idx] != 0) {
     ...
     * points are the same), and false if it is not
     */
    public static boolean isClosedPolygon(List<TripKitLatLng> poly) {
        TripKitLatLng firstPoint = poly.get(0);
        TripKitLatLng lastPoint = poly.get(poly.size() - 1);
     ...
     */
    public static double distanceToLine(final TripKitLatLng p, final TripKitLatLng start, final TripKitLatLng end) {
        if (start.equals(end)) {
        ...
        TripKitLatLng sa = new TripKitLatLng(p.latitude - start.latitude, p.longitude - start.longitude);
        TripKitLatLng sb = new TripKitLatLng(u * (end.latitude - start.latitude), u * (end.longitude - start.longitude));
     ...
     */
    public static List<TripKitLatLng> decode(final String encodedPath) {
        int len = encodedPath.length();

        // For speed we preallocate to an upper bound on the final length, then
        // truncate the array before returning.
        final List<TripKitLatLng> path = new ArrayList<TripKitLatLng>();
        int index = 0;
        int lat = 0;
        int lng = 0;
        ...
        while (index < len) {
            ...
            path.add(new TripKitLatLng(lat * 1e-5, lng * 1e-5));
        }
     ...
     */
    public static String encode(final List<TripKitLatLng> path) {
        long lastLat = 0;
        long lastLng = 0;

        final StringBuffer result = new StringBuffer();

        for (final TripKitLatLng point : path) {
