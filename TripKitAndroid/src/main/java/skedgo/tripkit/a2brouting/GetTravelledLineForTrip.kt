...
class GetTravelledLineForTrip @Inject constructor() {
  ...
  private fun createTravelledLinesToDraw(segments: List<TripSegment>?): List<List<LineSegment>> {
    return segments.orEmpty().filter { it.from != null && it.to != null }
        .map { segment ->
          ...
          listOf(lineSegmentsFromShapes, lineSegmentsFromStreets,
              listOf(LineSegment(TripKitLatLng(from.lat, from.lon), TripKitLatLng(to.lat, to.lon), color)))
              .first {
                it.isNotEmpty()
              }
        }
